...
- [x] Cloudflare D1 도입 — 접수중 공고 조회를 실시간 API 호출에서 DB 조회로 전환
      ...
      상태 자체가 존재하지 않음.
- [x] /sale·/rent를 "이것만 받기"에서 켬/끔 토글로 변경 (2026-07-20) — 예전엔 /sale을
      보내면 분양만 남고 임대 구독이 사라졌음(선택형). 이제 /sale·/rent(=/분양·/임대)는
      각 카테고리를 독립적으로 켜고 끄는 토글이라 둘 다 켜기/끄기/하나만 켜기가 전부
      가능. worker.js `CATEGORY_TOGGLE`(분양·임대 각각 토글) + `ALL_RESET`(/all·/전체 —
      둘 다 켜서 기본 상태로 리셋). 둘 다 켜지면 `subs.subscriptions[chatId]`를
      삭제해 "미설정 = 전체 수신"과 동일하게 취급, 둘 다 꺼지면 빈 배열로 저장(알림
      없음 — `chungyak_alert.py`의 `_targets()`가 이미 빈 배열=미구독으로 처리해
      Python 쪽은 수정 불필요). `/status` 안내 문구도 갱신.
