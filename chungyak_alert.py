...
선택 환경변수 3개 (Cloudflare D1 동기화 — 없으면 조용히 건너뜀):
    CF_ACCOUNT_ID     Cloudflare 계정 ID
    CF_D1_DATABASE_ID D1 데이터베이스 ID (chungyak-listings)
    CF_API_TOKEN      D1 Edit 권한 API 토큰
...
# D1(Cloudflare)에 "접수중 공고"만 미러링 — 텔레그램 /list가 data.go.kr을 직접 안 부르게 함
CF_ACCOUNT_ID = os.environ.get("CF_ACCOUNT_ID")
CF_D1_DATABASE_ID = os.environ.get("CF_D1_DATABASE_ID")
CF_API_TOKEN = os.environ.get("CF_API_TOKEN")
# 라이트 모드: 무순위만 자주 폴링하는 주간 보조 실행 (접수기간이 짧아 하루 2회론 놓침).
# LH 수집과 heartbeat는 생략, 명령 처리·리마인더·경쟁률은 그대로 (플래그로 중복 방지됨).
LIGHT = bool(os.environ.get("LIGHT_MODE"))
...
# ── D1 미러링 (텔레그램 /list가 data.go.kr 대신 조회하는 "접수중 공고" 전용 사본) ──
    ...
    try:
        ...
        for i in range(0, len(active), batch):
            ...
            for key, meta in chunk:
                ...
                params += [key, source, meta.get("type"), meta.get("name"), meta.get("region"),
                           meta.get("rcept_bgnde"), meta.get("rcept_endde"), meta.get("spsply_bgnde"),
                           meta.get("spsply_endde"), meta.get("przwner_de"), meta.get("url"), now]
            d1_query(sql, params)
    except Exception as e:
        print(f"⚠️ D1 동기화 실패: {e}", file=sys.stderr)
...
def main() -> None:
    ...
    items = collect()
    if not items:
        if LIGHT:   # 라이트 모드는 무순위만 봐서 0건이 정상일 수 있음
            print("✅ (light) 무순위 공고 없음 — 완료.")
            return
    ...
    lh_note = "skip" if lh_new is None else f"{lh_new}건"
    reminder_cnt = (1 if buckets else 0) + (1 if lh_due else 0)
    summary = (f"✅{' (light)' if LIGHT else ''} 새 공고 {len(new_items)}건, "
               f"리마인더 {reminder_cnt or '없음'}{'건' if reminder_cnt else ''}, "
               f"경쟁률 {len(cmpet_msgs)}건, LH 임대 {lh_note} — 완료.{skipped}")
    print(summary)
    # heartbeat: 아무 메시지도 안 나간 실행이면 한 줄 전송 — "안 옴 = 미실행"으로 구분 가능
    # (라이트 실행은 하루 4회라 heartbeat 생략 — 풀 실행 2회가 생존 신호)
    if not LIGHT and not (new_items or buckets or lh_due or cmpet_msgs or lh_new):
